...
def html_code_table(prod_df, table_name, file_name, side):
    ...
    table_head_row = '<tr><th>Product Category</th><th>Rating</th></tr>'

    html_code = table_style + table_head + table_head_row

    for i in range(len(prod_df.index)):
        row = '<tr><td>' + str(prod_df['Category'][i]) + '</td><td>' + str(prod_df['rating'][i]) + '</td></tr>'
...
def most_popular_table():
    most_popular_prods = prod_ranking_model.sort_values('Popularity_Rank', ascending=True)[['Product', 'Rate']].head(
...
def top_sell_table():
    top_sell_prods = prod_ranking_model.sort_values('Top_Sell_Rank', ascending=True)[['item_id', 'rating']].head(
        10).reset_index(drop=True)
...
def cust_most_popular_table(cust_name):
    ...
    cust_most_popular_prods = cust_most_popular_prods.sort_values('Popularity_Rank', ascending=True)[
        ['Product', 'Rate']].head(10).reset_index(drop=True)
...
def cust_top_sell_table(cust_name):
    cust_top_sell_prods = cust_prod_ranking_model[cust_prod_ranking_model['category'] == cust_name]
    cust_top_sell_prods = cust_top_sell_prods.sort_values('Top_Sell_Rank', ascending=True)[['category', 'rating']].head(
        10).reset_index(drop=True)
...
def recommend_prod_cust(cust_name):
    ...
    for i in range(len(similar_custs_corr)):
        if similar_custs_corr.index[i] != cust_name:
            cust_top_sell_prods = cust_prod_ranking_model[
                cust_prod_ranking_model['category'] == similar_custs_corr.index[i]]
            cust_top_sell_prods = cust_top_sell_prods[['category', 'item_id', 'rating']].reset_index(drop=True)
            cust_top_sell_prods['Qty_Corr'] = cust_top_sell_prods['rating'] * similar_custs_corr.iloc[i]
            prod_by_similar_custs = pd.concat([cust_top_sell_prods, prod_by_similar_custs])

    # aggregate the Qty Correlation by Product
    prod_by_similar_custs = prod_by_similar_custs.groupby('category').agg({'Qty_Corr': 'sum', 'Rate': 'max'})
    ...
    df_merge = pd.merge(prod_by_similar_custs, input_cust_top_sell_prods[['Product', 'No_of_Orders']], how='left',
                        on='Product')
    prod_recommend_to_cust = df_merge[df_merge['No_of_Orders'].isnull()]

    # sort the dataframe on Qty_Corr
    prod_recommend_to_cust = prod_recommend_to_cust.sort_values('Qty_Corr', ascending=False)[['category', 'rating']].head(
        10).reset_index(drop=True)
...
def similar_prods(prod_name):
    ...
    similar_prods = pd.merge(similar_prods_corr, prod_ranking_model[['category', 'rating']], how='left', on='item_id')

    prod_price = similar_prods[similar_prods['category'] == prod_name]['rating'].values[0]

    input_prod_index = similar_prods[similar_prods['category'] == prod_name].index
    similar_prods.drop(index=input_prod_index, inplace=True)

    similar_prods = similar_prods[['category', 'rating']].head(10).reset_index(drop=True)

    # print(similar_prods)

    html_code_table(similar_prods, 'Customers who purchased this product also purchased these', 'similarprodtable',
                    'left')
